...
function resolveModelName(model: YandexGptModel): string {
  ...
  // ВТОРАЯ ПОПЫТКА Pro 5.1 (19.09, после отката 0a7d5cb): первая попытка
  // на 3-сценарном regression не поймала баг — модель вместо structured
  // tool call написала псевдо-вызов инструмента текстом ("что делать
  // если клиент не пришёл на сессию?" → "```search_knowledge_base
  // {...}```" в ответе психологу). С тех пор добавлен responseGuard.ts —
  // барьер, блокирующий такой текст ПЕРЕД показом психологу (см. тот
  // файл). Сейчас включаем Pro 5.1 повторно на РАСШИРЕННОМ тесте
  // (15-20 вопросов, включая многократный повтор сломавшегося вопроса)
  // при единственном текущем пользователе (сам разработчик) — риска для
  // реальных бета-тестеров нет. Если баг не повторится на расширенном
  // наборе — оставляем на старт беты. Если повторится хотя бы раз —
  // откат на Pro 5, Pro 5.1 ждёт до после беты с более основательным
  // архитектурным фиксом (не быстрой правкой в последние часы).
  //
  // Официальная документация Yandex (aistudio.yandex.ru/ru/docs/ai-studio/
  // concepts/generation/models, сверено 19.09): алиас "yandexgpt/latest"
  // указывает на Pro 5, явное имя Pro 5.1 — "yandexgpt-5.1". Pro 5.1
  // стоит 0.8₽/1000 токенов вход/кэш/исход против 1.2₽/1000 у Pro 5 —
  // ~33% дешевле на этой части стоимости запроса.
