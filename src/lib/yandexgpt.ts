...
function resolveModelName(model: YandexGptModel): string {
  ...
  // Pro 5.1 ОТЛОЖЕНА ДО ПОСЛЕ БЕТЫ (решение от 19.09, вторая попытка).
  // По прайсу Pro 5.1 стоит 0.8₽/1000 токенов вход/кэш/исход против
  // 1.2₽/1000 у Pro 5 (~33% дешевле), но обе попытки включения в проде
  // (334a189, затем 3ffcc2e после добавления responseGuard.ts) поймали
  // один и тот же класс бага на первом же вопросе "Что делать если
  // клиент не пришёл на сессию?": модель пишет псевдо-tool-call текстом
  // вместо structured toolCallList.toolCalls. Второй раз паттерн был
  // ДРУГОЙ формы (вложенный JSON {"role":"assistant","message":{"name":
  // ...}}) и прошёл мимо responseGuard.ts — то есть это не единичный
  // текстовый паттерн, а системное поведение модели на API-уровне,
  // требующее либо разделения агентского цикла на tool-стадию и
  // synthesizer-стадию (см. задачу #21, отложена как архитектурно
  // нетривиальная), либо более общего fallback-парсинга. Не годится для
  // правки в последние часы перед стартом беты. Критерий пользователя:
  // при повторе бага — откат и отсрочка до после беты. Оба отката
  // выполнены (0a7d5cb, 077514f). Дефолт — снова "yandexgpt/latest"
  ...
  // UUID-баг).
  // Pro 5.1 остаётся отложенной до после беты (см. подробный комментарий
  // в истории коммитов 19-20.09). Кратковременное включение для сбора
  // паттернов псевдо-tool-call (задача #26, ночь 19→20.09) остановлено
  // на середине из-за исчерпанного месячного лимита запросов ассистента
  // тестового аккаунта (200/200) — сброс лимита через код заблокирован
  // permission-классификатором как изменение shared-ресурса. Собранные
  // на этот момент данные see docs/pro-5.1-pattern-collection-notes (или
  // отчёт в чате) — 3 успешных прогона проблемного вопроса, 2 из 3 снова
  // сломались (один поймал responseGuard, один прошёл сырым JSON). Прод
  // возвращён на стабильную "yandexgpt/latest" (Pro 5) на ночь.
