...
function AskAnythingDemo() {
  const tabs = [
    ...
    },
    {
      ...
      color: "#1BAF7A",
      questions: [
        "Оформи протокол сессии по SOAP",
  ...
  const answers: Record<string, { text: string; chart?: Chart }> = {
    ...
    "Оформи протокол сессии по SOAP": {
      text: "Протокол готов и сохранён в карточку клиента. Subjective — жалобы на тревожность перед встречами с руководством; Objective — спокойная речь, открытая поза, среднее число пауз; Assessment — устойчивый прогресс в работе с перфекционизмом; Plan — продолжить экспозиционные техники, следующая методика — PSS-10.",
    },
    "Повторяющиеся темы за последние 3 месяца": {
...
function HowItWorksInteractive() {
  ...
  const steps = [
    ...
    },
    {
      ...
      icon: <IconBrain />,
      details: ["Анализ тона и эмоций", "Выделение ключевых моментов", "Предложения для SOAP"],
    },
    {
...
function FeaturesCardStack({ featureId, cardIndex, onCardChange, onBack }: any) {
  ...
  const featureContent: Record<string, { details: string; howTo: string; pros: string }> = {
    "ai-notes": {
      details: "Пока вы работаете с клиентом, платформа фиксирует сессию и структурирует её содержание. Через 2 минуты после звонка — готовый протокол по SOAP.",
      howTo: "1. Проведите сессию как обычно\n2. Платформа транскрибирует разговор\n3. Через 2 минуты протокол готов\n4. Редактируйте и экспортируйте в PDF или Word",
      pros: "✓ SOAP-протокол без ручной работы\n✓ Экспорт в PDF и Word\n✓ 20 часов в месяц освобождается",
    },
    "ai-assistant": {
    ...
    "ai-plan": {
      details: "Каждый протокол завершается блоком Plan — что наметили на следующую встречу. Вы приходите на следующую сессию уже в полном контексте.",
      howTo: "1. После сессии протокол формируется автоматически\n2. Блок Plan содержит план на следующую встречу\n3. При необходимости отредактируйте\n4. Используйте на следующей сессии без подготовки",
    ...
    "forms": {
      details: "Более 30 валидированных методик: PHQ-9, GAD-7, WHO-5, DASS-21 и другие. Клиент проходит тест прямо в мессенджере.",
    ...
    "capture": {
      details: "Платформа фиксирует сессию и транскрибирует её с точным разделением: что говорили вы, что говорил клиент.",
      howTo: "1. Начните сессию на платформе\n2. Запись идёт автоматически в фоне\n3. Точное разделение реплик: вы и клиент\n4. Транскрипт используется для протокола",
      pros: "✓ Ничего не нужно запускать вручную\n✓ Точное разделение реплик\n✓ Основа для протокола SOAP",
    },
    "telehealth": {
...
export default function LandingPage() {
  ...
  const features = [
    ...
    },
    {
      ...
      desc: "Система генерирует структурированный протокол по SOAP за 2 минуты. Вы редактируете и экспортируете.",
      color: "#F59E0B",
  ...
  const testimonials = [
    {
      ...
      role: "Психолог, КПТ",
      rating: 5,
      text: "Раньше после каждой сессии уходило по 40–60 минут на конспект. Теперь протокол по SOAP готов через 2 минуты — я только проверяю и редактирую. Освободилось около 20 часов в месяц.",
      avatar: "МС",
      color: "#2D6A5C",
    },
    {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
Проведите сессию как обычно — <strong style={{ color: "#1C1C1E", fontWeight: 700 }}>платформа сама превратит её в готовый протокол</strong>, обновит карточку клиента и отправит ему напоминание. Вы просто проверяете и переходите к следующей встрече.
            </p>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          >
            <table style={{ width: "100%", minWidth: 640, fontSize: 13 }}>
              ...
              <tbody>
                {[
                  ["Протокол сессии (SOAP) за 2 мин", "✓ авто", "✓ AI-мемо", "✓ резюме"],
